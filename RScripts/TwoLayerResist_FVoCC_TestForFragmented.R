...
cores = detectCores()
ncores = cores[1]-1
#Cut it by cores and number of batches
cuts <- cut(1:nrow(dat), ncores*batch_total, labels = FALSE) #cuts[number of row] is which number of batch this row belongs to.
                    ...
                    if(out.lcd$cost.dist == 0) {
                      ...
                      resu[i, vel_km_yr := NA]
                        ...
                        if (length(from.index) > 1) {
                          from.index[1] #if all the values are Inf in cost.distance1, it just pick the first anacid.
                          ##> from.index[1]
                          ##[1] 1
...
batch_1 <- raster(pre1)
batch_1[result$focal] <-  result$vel_km_yr
plot(batch_1)
...
writeRaster(batch_1, filename = "/vsc-hard-mounts/leuven-data/348/vsc34871/VoCC/Output/FVoCC_TwolayersResist_FramentedFixed_UK_CHELSA_EPSG3035_ssp126_batch01.tif", format="GTiff", overwrite=TRUE)
